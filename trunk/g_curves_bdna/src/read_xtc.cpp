...
void  read_xtc2structure(char * filename,char * out_file,char * choose,int base_1_serial,int base_2_serial,struct atom * atom_head,int step_frame )
{
	...
	//**********************	
		...
		struct base_pyrimidine_serial * by_1;
		struct base_pyrimidine_serial * by_2;
...
//得到碱基的原子的序号。
		if((ch_b_1->base_name=='A')||(ch_b_1->base_name='G'))
		{
		...
		{
			by_1=read_pdb2pyrimidine_base(atom_head,*ch_b_1);
			b3=true;
		}

		if((ch_b_2->base_name='A')||(ch_b_2->base_name='G'))
		{
		...
		{
			by_2=read_pdb2pyrimidine_base(atom_head,*ch_b_2);
			b4=true;
		}
        ...
        while(1)
       {
			...
			if(num_step%step_frame==0)
			{
					...
					if(b3==true)
					{
						q_1_matrix=dmatrix(0,6,0,2);
					}
					if(b4==true)
					{
						q_2_matrix=dmatrix(0,6,0,2);
					}
          	...
          	#ifdef DEBUG
		//	
		 //	cout<<step<<"\t"<<time_temp<<"\t"<<i<<"\t"<<x[i][0]<<"\t"<<x[i][1]<<"\t"<<x[i][2]<<endl;
		// 	oop<<step<<"\t"<<time_temp<<"\t"<<i<<"\t"<<x[i][0]<<"\t"<<x[i][1]<<"\t"<<x[i][2]<<endl;
		#endif		
					 //q_1_G_1
					if(b1==true)
					{
...
//
			...
			if(b3==true)
			{
				Rotation_matrix(q_1_matrix,'C',rotation_matrix_1);
				origin_vector(q_1_matrix,rotation_matrix_1,'C',o_vector_1);
			}
			if(b4==true)
			{
				Rotation_matrix(q_2_matrix,'C',rotation_matrix_2);
				origin_vector(q_2_matrix,rotation_matrix_2,'C',o_vector_2);
			}
