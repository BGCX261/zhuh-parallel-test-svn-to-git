...
//求取数据分布的函数
void Distribution(double *sd,int N,string filename,string para)
{
	ofstream outfile;
	double  *sf;
	double  *ds;
	...
	double MAX_a=0,MIN_a=0;
	double MAX_b=sd[0],MIN_b=sd[0];
	double MAX=0,MIN=0;
	double total=0;
/*给定最大值和最小值*/
	if(para=="shift")
	...
	if(para=="chi"||para=="gamma"||para=="delta"||para=="epsil"||para=="zeta"||para=="alpha"||para=="beta")
	{
		MIN_a=-180;
		MAX_a=180;
	}
	if(para=="c1"||para=="c2"||para=="c3")
	{
		MIN_a=90;
		MAX_a=120;
	}
